---
title: "STAT545A_MiniDataAnalysis1"
author: "Torin"
date: '2023-10-03'
...
<!-------------------------- Start your work below ---------------------------->
_The following code will discover some key attributes of each dataset._
I figured out how to write a simple function which will return some of the key attributes of a dataset. The function will return:
1. An overview of the dataset using `glimpse`
...
_Now, I apply the function to each of the datasets, starting with cancer_sample:_
...
<!-------------------------- Start your work below ---------------------------->
I will use the _cancer_samples_ dataset. My research is in immunology and I have an interest in cancer immunotherapies, so this looks like a fun dataset to explore further. It also appears to be the largest and most complex of these four datasets, which will prepare me better for analysis of large-scale data in the future.
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
I would like to address the question of whether the two diagnoses, 'M' and 'B', differ in their physical attributes such as size, shape and texture, that have been measured here.
<!----------------------------------------------------------------------------->
...
1. Plot the distribution of a numeric variable.
  - I am choosing this task because my dataset is largely composed of numeric variables, and this could be a good way to familiarize myself with any possible trends in these variables
  
_The following code will plot the distribution of radius_mean_
```{r}
library(ggplot2)
...
4. Explore the relationship between two numeric variables
  - I am choosing to explore the relationship between radius_mean and area_mean, as this will give some insights into the shape of the tumour, at least in two dimensions
  
_The following code will plot the relationship of radius_mean and area_mean, coloured by diagnosis._
```{r}
...
This reveals a non-linear relationship between radius and area, which could be consistent with a circular or spherical tumour shape. There is a very clear trend where cancers with the 'B' diagnosis are consistently smaller than those with the 'M' diagnosis.

7. Make a new tibble with a subset of your data, with variables and observations that you are interested in exploring.
  - I will make a new tibble looking at only the columns containing mean observations, which simplifies the dataset. I will also filter out observations with missing diagnoses, as this is not relevant to my research question here, as well as those with missing patient ID.
```{r}
...
8.  Use a density plot to explore any of your variables (that are suitable for this type of plot).
  - This was chosen to visualize the data based on the two diagnosis groups, 'M' and 'B'
  
_The following code will plot the mean texture density distribution for the 'M' and 'B' diagnosis groups._
```{r}
library(ggridges)
...
4. What is the largest and smallest tumour in the 'B' and 'M' groups based on area, perimeter and radius?
<!----------------------------->
