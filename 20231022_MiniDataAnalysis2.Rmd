...
## Do ‘M’ and ‘B’ tumours differ in their symmetry?

1. Compute the *range*, *mean*, and *two other summary statistics* of **one numerical variable** across the groups of **one categorical variable** from your data.

```{r}
#Change this - compute the number of observations.
#This code will compute the range, mean, median and standard deviation of the symmetry across the 'B' and 'M' groups.
head(cancer_sample)
cancer_sample %>%
  group_by(diagnosis) %>%
  summarize(min_symmetry = min(symmetry_mean), max_symmetry = max(symmetry_mean), mean_symmetry = mean(symmetry_mean), median_symmetry = median(symmetry_mean), standarddeviation_symmetry = sd(symmetry_mean))
#As we can see, the output shows that there are actually only very small differences in symmetry between the two groups, although we would need proper statistical analysis to assess the significance of these differences.
```
8. Create 3 histograms, with each histogram having different sized bins. Pick the "best" one and explain why it is the best.
```{r}
...
```{r}
#The following code will divide tumour size into large, medium and small groups, creating a new categorical variable 'compactness_cat'
```

7. Create a graph that has at least two geom layers. 

```{r}
#The following code will create boxplots of tumour compactness, showing the median and interquartile range, for each tumour size category. A second geom layer, points, will be overlaid on the histogram and will visualize differences in the compactness distribution between 'B' and 'M' tumours using different colours.
```
...
```{r}
#The following code will create scatterplots of each numerical variable against the fractal dimension variable, to visualize potential relationships between the fractal dimension and other variables. 
```
...
```{r}
#The following code will compute the range, mean, median and standard deviation for the area, perimeter and radius across the 'B' and 'M' groups of the diagnosis variable.
```

6. Make a graph where it makes sense to customize the alpha transparency.

```{r}
#The following code will create boxplots of the area, perimeter and radius for the 'B' and 'M' groups, showing the smallest observation in each group as a dark point to allow this observation to stand out.
```
<!----------------------------------------------------------------------------->
